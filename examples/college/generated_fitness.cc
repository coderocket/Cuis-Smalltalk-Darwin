b->fitness += sum(b->gene, b->gene+CHROMOSOME_SIZE, [](const gene_t* each) { return 10; });


{ int keys[] = {GENIE_DAY, GENIE_LECTURER}; 

b->fitness -= group_by(b, keys,2,[](gene_t** b, gene_t** e) 
	{ return  ( max ( (  ( ( genie_sum(b,e,[](const gene_t* each) { return (*each)[GENIE_DURATION]; }) ) - ( 5 ) )  ) , ( 0 ) ) ) ; }); }
...
{ int keys[] = {GENIE_DAY, GENIE_FRAME}; 

b->fitness -= group_by(b, keys,2,[](gene_t** b, gene_t** e) 
	{ return  ( max ( (  ( ( genie_maxgap(b,e,[](const gene_t* each) { return  interval_t ( (*each)[GENIE_HOUR] ,  ( (*each)[GENIE_HOUR] ) + ( (*each)[GENIE_DURATION] )  ) ; }) ) - ( 3 ) )  ) , ( 0 ) ) ) ; }); }
{ int keys[] = {GENIE_FRAME}; 

b->fitness -= group_by(b, keys,1,[](gene_t** b, gene_t** e) 
	{ return  ( max ( (  ( ( genie_count(b,e,[](const gene_t* each) { return (*each)[GENIE_DAY]; }) ) - ( 4 ) )  ) , ( 0 ) ) ) ; }); }
...
b->fitness -= sum(b->gene, b->gene+CHROMOSOME_SIZE, [](const gene_t* each) { return  ( max ( (  ( (  ( (  ( *each ) [ GENIE_HOUR ]  ) + (  ( *each ) [ GENIE_DURATION ]  ) )  ) - ( 17 ) )  ) , ( 0 ) ) ) ; });
{ int keys[] = {GENIE_DAY, GENIE_FRAME}; 
